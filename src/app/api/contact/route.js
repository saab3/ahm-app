...
export async function POST(request) {
  ...
  if (!data.email || !data.name || !data.message || !data.subject) {
    return new Response("Error Occured, please fill the form correctly", {
      status: 403,
    });
  }
  try {
    await transporter.sendMail({
      from: `<${process.env.NEXT_PUBLIC_USER_SENDER}>`,
      to: process.env.NEXT_PUBLIC_USER_RECIEVER,
